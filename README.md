...
**Stealth mode** makes OTTO invisible to screen-capture tools — independent of Privacy Lock, built for using OTTO alongside proctored/browser-based sessions (CoderPad, Google Meet, Zoom). Turning it on excludes OTTO's window from every conformant capture path — the legacy `NSWindow.sharingType` flag *and* the window-server `CGSSetWindowCaptureExcludeShape` call that also defeats ScreenCaptureKit / browser `getDisplayMedia` — floats it above other windows, and hides the menu bar + Dock icons, while keeping the normal window otherwise unchanged. Caveat: capture exclusion uses a private macOS API (App Store-ineligible, may change across macOS versions); Apple's own QuickTime and blessed conferencing partners can still capture the window.

**Compact mode** is a separate preference, only offered once Stealth mode is on, that additionally hides the normal window and moves OTTO into **two borderless, non-activating `NSPanel`s** — a **Chat** panel (compact composer with an expandable message history) and a separate movable **Live Capture** panel (audio transcription + screenshots, whose "Ask Otto" hands off to the Chat panel), both toggled with `⌘⇧\` — so interacting with OTTO never deactivates the browser and trips focus-loss detection. Turning stealth off also turns compact off. Both toggles live under Settings → Privacy & Security.
