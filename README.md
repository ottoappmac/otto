...
**Stealth mode** makes OTTO invisible to screen-capture tools and adds a focus-safe overlay — independent of Privacy Lock, built for using OTTO alongside proctored/browser-based sessions (CoderPad, Google Meet, Zoom). Turning it on (a) excludes OTTO's windows from every conformant capture path — the legacy `NSWindow.sharingType` flag *and* the window-server `CGSSetWindowCaptureExcludeShape` call that also defeats ScreenCaptureKit / browser `getDisplayMedia` — and (b) hides the normal window and moves OTTO into **two borderless, non-activating `NSPanel`s** — a **Chat** panel (compact composer with an expandable message history) and a separate movable **Live Capture** panel (audio transcription + screenshots, whose "Ask Otto" hands off to the Chat panel), both toggled with `⌘⇧\` — so interacting with OTTO never deactivates the browser and trips focus-loss detection. It also hides the menu bar + Dock icons. Caveat: capture exclusion uses a private macOS API (App Store-ineligible, may change across macOS versions); Apple's own QuickTime and blessed conferencing partners can still capture the window. Both toggles live under Settings → Privacy & Security.
